...
from .pipeline import QUEUE_BUCKETS, queue_buckets_for_job
...
def get_queue(state_dir: str | Path) -> Dict[str, List[Dict[str, str]]]:
    ...
    Buckets:
        ...
        unsubmitted   -- status waits for submission
    ...
    buckets: Dict[str, List[Dict[str, str]]] = {bucket: [] for bucket in QUEUE_BUCKETS}
    buckets.update({
        "opp_candidate": [],
        "opp_verifying": [],
        "opp_planning": [],
    ...
    for job_id, job in state.get("jobs", {}).items():
        entry = _entry(job_id, job)
        for bucket in queue_buckets_for_job(job):
